...
export function useChartData() {
    ...
    useEffect(() => {
        ...
        if (largerCandleData.length) {
            ...
            if (
                from &&
                from.index - largerCandleData[0].index <= 300 &&
                largerCandleData[0].index
            ) {
                setNewRequest((previous) => ({
                    start:
                        largerCandleData[0].index - 300 <= 0
                            ? 0
                            : largerCandleData[0].index - 300,
            ...
            if (
                to &&
                largerCandleData[largerCandleData.length - 1].index -
                    to.index <=
                    100
            ) {
                setNewRequest((previous) => ({
                    start:
                        largerCandleData[largerCandleData.length - 1].index + 1,
                    end:
                        largerCandleData[largerCandleData.length - 1].index +
                        101,
                }));
