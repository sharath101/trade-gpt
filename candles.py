...
class CandleManager:
    ...
    def process_tick(self, timestamp, price, symbol, save_redis = 1):
        ...
        if symb['last_timestamp'] is None:
            symb['last_timestamp'] = current_time - timedelta(minutes=current_time.minute % self.interval_minutes,
                                                         seconds=current_time.second)
        if symb['current_candle'] is None:
            self._open_candle(symb['last_timestamp'], price, symbol)
        
        while symb['last_timestamp'] + timedelta(minutes=self.interval_minutes) <= current_time:
        ...
        if save_redis:
            redis_instance.set(f'candle_{symbol}_{self.interval_minutes}', symb['candles'])

    def _open_candle(self, timestamp, price, symbol):
        symb = self.symbols[symbol]
        symb['current_candle'] = {'timestamp': timestamp,
                               'open': price,
                               'high': price,
                               'low': price,
                               'close': price}
    ...
    def _close_candle(self, symbol):
        symb = self.symbols[symbol]
        symb['candles'].append({'timestamp': symb['current_candle']['timestamp'],
